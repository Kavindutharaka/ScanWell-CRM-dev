...
namespace back_end.Controllers
{
    ...
    public class QuoteCopyController : ControllerBase
    {
        ...
        public QuoteCopyController(IConfiguration configuration)
        {
        ...
        public IActionResult CreateQuote([FromBody] QuoteCopy quote)
        {
            string query = @"
            INSERT INTO [dbo].[quotes_save] 
            (quoteId, customerId, customerName, pickupLocationId, deliveryLocationId, creditTermsId, createdDate, clientId, clientName, days, rateValidity, freightMode, freightCategory, createdBy, directRoute, transitRoute, multimodalSegments, routePlanData, freightCharges, handlingCharges, termsConditions, customTerms)
            VALUES 
            (@quoteId, @customerId, @customerName, @pickupLocationId, @deliveryLocationId, @creditTermsId, @createdDate, @clientId, @clientName, @days, @rateValidity, @freightMode, @freightCategory, @createdBy, @directRoute, @transitRoute, @multimodalSegments, @routePlanData, @freightCharges, @handlingCharges, @termsConditions, @customTerms)";
            ...
            using (myCon)
            {
                ...
                using (myCom = new SqlCommand(query, myCon))
                {
                    ...
                    myCom.Parameters.AddWithValue("@rateValidity", quote.rateValidity ?? (object)DBNull.Value);  // Added parameter
        ...
        [HttpPut, Route("quote")]
        public IActionResult UpdateQuote([FromBody] QuoteCopy quote)
        {
            ...
            UPDATE [dbo].[quotes_save] SET
                ...
                days = @days,
                rateValidity = @rateValidity,  -- Added field
