...
export default function HandlingChargesSection({ formData, setFormData, type, title, disabled = false }) {
  ...
  const addCharge = () => {
    ...
    setFormData(prev => ({
      ...prev,
      [chargeField]: [...prev[chargeField], { chargeName: '', unitType: '', numberOfUnits: '', amount: '', currency: '', total: 0, remark: '' }]
    }));
  };
  ...
  const removeCharge = (index) => {
    ...
    setFormData(prev => ({
      ...prev,
      [chargeField]: prev[chargeField].filter((_, i) => i !== index)
    }));
  };
  ...
  const suggestions = type === 'origin' ? chargeNameSuggestions.origin : chargeNameSuggestions.destination;
