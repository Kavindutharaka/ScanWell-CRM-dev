// components/SeaFreightRatioTable.jsx
import { Plus, Trash2 } from 'lucide-react';
import AutocompleteInput from './AutocompleteInput';
import { getCarriersByCategory, currencySuggestions } from '../../../data/quoteData';
import { useState, useEffect } from 'react';

export default function SeaFreightRatioTable({ formData, setFormData, disabled = false, fieldName = 'seaFreightRatioCharges' }) {
  ...
  const [horizontalData, setHorizontalData] = useState(() =>
    transformToHorizontal(formData[fieldName] || [])
  );

  useEffect(() => {
    const verticalCharges = transformToVertical(horizontalData);
    setFormData(prev => ({ ...prev, [fieldName]: verticalCharges }));
  }, [horizontalData]);
  ...
  useEffect(() => {
    ...
    if (currentData.length > 0) {
      ...
      const newVertical = JSON.stringify(currentData);

      if (currentVertical !== newVertical) {
        setHorizontalData(transformToHorizontal(currentData));
      }
    }
  }, [formData[fieldName]]);
