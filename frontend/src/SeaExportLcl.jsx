...
export default function SeaExportLcl({ 
  ...
  const tableColumnWidths = [15, 15, 15, 15]; // LINER, FREIGHT RATE, ROUTING, TRANSIT TIME
  ...
  const tableHeader = [
    ...
    padString("FREIGHT RATE", tableColumnWidths[1], 'left'),
...
Kindly note that Scanwell’ Logistics will accept payments in ${currency} currency for shipments`;
  ...
  useEffect(() => {
    if (autoCopy) {
      const timer = setTimeout(() => {
        const div = document.querySelector('.sea-export-lcl-template');
        if (div) {
          copyToClipboard(div);
        }
      }, 100);
  ...
  return (
    <div 
      ref={(el) => { if (el && autoCopy) el.classList.add('sea-export-lcl-template'); }}
      onClick={handleCopy} 
      role="button" 
      tabIndex={0} 
    ...
    >
      ...
      <h1 className='uppercase font-bold underline text-2xl mb-4'>SEA EXPORT LCL</h1>
      ...
      <table className='border-2 border-collapse w-full mb-4'>
        <thead>
          <tr>
            {["LINER", "FREIGHT RATE", "ROUTING", "TRANSIT TIME"].map((header, index) => (
